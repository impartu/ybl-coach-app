...
import { CalendarClock, RotateCcw, Download, AlertCircle, Wand2, HelpCircle, Printer, Save, Bookmark, BookmarkPlus, Trash2, FileJson, ChevronDown, ChevronUp, Settings } from 'lucide-react';
// @ts-ignore
import html2canvas from 'html2canvas';
import { db } from './firebase';
import { collection, addDoc, getDocs, deleteDoc, doc, getDoc, setDoc, updateDoc, serverTimestamp, query, orderBy } from 'firebase/firestore';
import { INITIAL_ROSTER, INITIAL_PLAYERS, PERIOD_NAMES } from './constants';
import { Player, RotationState, TOTAL_PERIODS, SLOTS_PER_PERIOD, MINUTES_PER_PERIOD, PlayerSeasonStats, AssignSettings, DEFAULT_ASSIGN_SETTINGS, SavedGame, SavedGamePlayer } from './types';
...
function App() {
  // --- State ---
  // Dynamic roster state initialized with 10 players
  const [roster, setRoster] = useState<Player[]>(() => JSON.parse(JSON.stringify(INITIAL_ROSTER)));
  ...
  const fetchSeasonData = async () => {
    ...
    try {
      ...
      // Sort games chronologically descending (newest first)
      gamesList.sort((a, b) => {
  ...
  const fetchFavorites = async () => {
    try {
      ...
      // Fallback query if orderBy index or field isn't present
      try {
        const fallbackSnapshot = await getDocs(collection(db, 'favorites'));
        const favs: FavoriteItem[] = [];
        ...
        console.error('Error fetching favorites:', fallbackErr);
      }
    }
  };
  ...
  const fetchRoster = async () => {
    try {
      const docRef = doc(db, 'team_settings', 'current_roster');
      const docSnap = await getDoc(docRef);
      if (docSnap.exists()) {
        const data = docSnap.data();

        // Load assignSettings if saved
        if (data?.assignSettings) {
          setAssignSettings({
            requireRoles: typeof data.assignSettings.requireRoles === 'boolean' ? data.assignSettings.requireRoles : false,
            extraMinutes: data.assignSettings.extraMinutes === 'competitive' ? 'competitive' : 'development',
            balanceDefense: typeof data.assignSettings.balanceDefense === 'boolean' ? data.assignSettings.balanceDefense : false,
            staggerDepth: typeof data.assignSettings.staggerDepth === 'boolean' ? data.assignSettings.staggerDepth : false,
          });
        }

        const loadedRoster = Array.isArray(data?.roster)
          ? data.roster
          : (Array.isArray(data?.players) ? data.players : null);
      ...
      console.error('Error fetching current_roster from team_settings:', error);
    }
  };
  ...
  const handleSaveFavorite = async () => {
    ...
    try {
      ...
      setSelectedFavoriteId(docRef.id);
    } catch (error) {
  ...
  const handleSaveGame = async () => {
    ...
    try {
      ...
      const roster = activePlayers.map(player => {
    ...
    try {
      ...
      querySnapshot.forEach((docSnap) => {
        ...
        if (data?.timestamp?.toDate) {
          ...
          formattedTimestamp = data.timestamp.toISOString();
